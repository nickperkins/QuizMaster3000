	...
	public void run() {
		...
		while (t == thisThread) {
			...
			case FINISHED:
				...
				break;
			case REGISTRATION:
				plugin.getServer().broadcastMessage(plugin.formatMessage("%sA new game of quiz has started. Type /quiz join to play! We start in 1 minute.", ChatColor.GOLD));
				try {
					Thread.sleep(1000 * 30);
					plugin.getServer().broadcastMessage(plugin.formatMessage("%s30 seconds until we start. Type /quiz join to play!", ChatColor.GOLD));
					Thread.sleep(1000 * 20);
					plugin.getServer().broadcastMessage(plugin.formatMessage("%s10 seconds until we start. Type /quiz join to play!", ChatColor.GOLD));
					Thread.sleep(1000 * 10);
					if (plugin.scores.size() == 0) {
						plugin.getServer().broadcastMessage(plugin.formatMessage("%sNo players joined the game. Maybe next time?", ChatColor.GOLD));
						plugin.state = QuizState.FINISHED;
					} else {
						plugin.getServer().broadcastMessage(plugin.formatMessage("%sJust type your answers into chat. Ready? Let's play!", ChatColor.GOLD));
						Thread.sleep(1000 * 5);
			...
			case ASKQUESTION:
				...
				try {
					...
					plugin.getServer().broadcastMessage(plugin.formatMessage("%ssQuestion: " + plugin.currentQuestion.getQuestion(), ChatColor.GOLD));
					Thread.sleep(1000 * 20);
					plugin.getServer().broadcastMessage(plugin.formatMessage("%s10 seconds left...", ChatColor.GOLD));
					plugin.getServer().broadcastMessage(plugin.formatMessage("%sQuestion: " + plugin.currentQuestion.getQuestion(), ChatColor.GOLD));
					Thread.sleep(1000 * 10);
				} catch (InterruptedException e) {
					break;
				...
				break;
			case GETANSWER:
				plugin.getServer().broadcastMessage(plugin.formatMessage("%sTime's up!", ChatColor.RED));
				plugin.getServer().broadcastMessage(plugin.formatMessage("%sThe correct answer was: %s", ChatColor.GOLD, StringUtils.join(Arrays.copyOfRange(plugin.currentQuestion.getAnswer(), 0, plugin.currentQuestion.getAnswer().length), " or ")));
				if (plugin.scores.size() == 0) {
					plugin.getServer().broadcastMessage(plugin.formatMessage("%sSeems all the players from this round have left. No point going on, is there?", ChatColor.GOLD));
					plugin.state = QuizState.FINISHED;
				} else {
					plugin.state = QuizState.WAITFORNEXT;
				}
				...
				break;
			case WAITFORNEXT:
				plugin.getServer().broadcastMessage(plugin.formatMessage("%sOK, Next Question...", ChatColor.GOLD));
				plugin.state = QuizState.ASKQUESTION;
				try {
					Thread.sleep(1000 * 5);
				} catch (InterruptedException e) {
